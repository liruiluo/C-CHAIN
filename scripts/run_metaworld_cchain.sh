...
# Run C-CHAIN PPO on 10 Meta-World goal-observable tasks,
...
for task in "${TASKS[@]}"; do
  for seed in "${SEEDS[@]}"; do
    echo "=== Running ${task}, seed ${seed}, total_steps=${TOTAL_STEPS} ==="
    uv run python crl_metaworld/crl_run_ppo_c_chain_metaworld.py \
      --env_name "${task}" \
